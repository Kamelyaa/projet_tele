...
function generateCalendar($year, $month) {
    ...
    for ($day = 1; $day <= date('t', $firstDayOfMonth); $day++) {
        ...
        if (($month == 5 && $day == 17) || ($month == 5 && $day == 22)) {
            // Si la date est le 17 mai ou le 22 mai, affichez le rendez-vous spécifique avec un lien
            $calendarHTML .= "<td><button onclick='handleDayClick($year, $month, $day)'>Rdv Mme Kamelya</button></td>";
        } else {
...
<body>
    ...
    <script>
        ...
        function handleDayClick(year, month, day) {
            if ((month == 5 && day == 17) || (month == 5 && day == 22)) {
                // Si la date est le 17 mai ou le 22 mai, affichez le rendez-vous spécifique avec un lien cliquable
                alert("Rdv Mme Kamelya\nCliquez sur le lien pour plus d'informations.");
                var link = document.createElement('a');
                link.href = 'http://localhost:3000';
                link.textContent = 'http://localhost:3000';
                link.target = '_blank'; // Ouvre le lien dans un nouvel onglet
                alert("Lien : http://localhost:3000");
            } else {
